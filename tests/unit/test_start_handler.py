...
async def test_cmd_start_active_user(monkeypatch):
    ...
    mock_user_authorization = AsyncMock(return_value=fake_user)
    monkeypatch.setattr("bot.handlers.start.user_authorization", mock_user_authorization)

    message = build_message(tg_id=123, username="tester")

    await cmd_start(message)

    mock_user_authorization.assert_awaited_once_with(user_tg_id=123, username="tester")
...
async def test_cmd_start_inactive_user(monkeypatch):
    ...
    mock_user_authorization.assert_awaited_once_with(user_tg_id=456, username="noactive")
...
async def test_cmd_start_without_username(monkeypatch):
    ...
    mock_user_authorization.assert_awaited_once_with(user_tg_id=789, username=None)
